...
prompt = ChatPromptTemplate.from_messages([
    ...
    ("user", "{input}"),
    ("user", "Given the above conversation, generate a search query to look up to get information relevant to the conversation. but answer all questions in the style of William Shakespear.")
])
retriever_chain = create_history_aware_retriever(chat, retriever, prompt)

prompt = ChatPromptTemplate.from_messages([
    ("system", "Answer the user's questions based on the below context in the style of William Shakespear:\n\n{context}"),
    MessagesPlaceholder(variable_name="chat_history"),
    ("user", "{input}"),
])
...
def build_message_list():
    ...
    zipped_messages = [SystemMessage(
        content="You are a helpful AI assistant talking with a human that only knows how to write as William Shakespear. If you do not know an answer, just say 'I don't know', do not make up an answer.")]
