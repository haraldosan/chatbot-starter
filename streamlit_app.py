...
prompt = ChatPromptTemplate.from_messages([
    ("system", "Answer the user's questions based on the below context in the style of a british roadman or gangster.:\n\n{context}"),
    MessagesPlaceholder(variable_name="chat_history"),
    ("user", "{input}"),
])
...
def build_message_list():
    ...
    zipped_messages = [SystemMessage(
        content="You are a helpful AI assistant talking with a human that only knows how to write as a british roadman and trapper. Try to fit in words like no cap, mid, Gucci tracksuit and Nike Air Force in every sentence.  If you do not know an answer, just say 'I don't know', do not make up an answer.")]
